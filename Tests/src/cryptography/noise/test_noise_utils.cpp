...
TEST(CryptographyNoiseUtils, initializeKey_testFromEmpty)
{
	const std::vector<uint8_t> keyData = hexToBytes("14aa993c4bb8cfc0893ac0be584bc41e48537c67cd31e0c0659ff18235c7cdd3");
	std::array<uint8_t, Cryptography::CipherKeySize> expectedKey = vectorToArray<Cryptography::CipherKeySize>(keyData);

	Cryptography::CipherKey newKey;
	vectorToArray(keyData, newKey.raw);

	Noise::CipherState cipherState;
	Noise::Utils::initializeKey(std::move(newKey), cipherState);

	EXPECT_EQ(cipherState.cipherKey.raw, expectedKey);
	EXPECT_EQ(cipherState.nonce, static_cast<uint64_t>(0));
}

TEST(CryptographyNoiseUtils, initializeKey_testFromExisting)
{
	const std::vector<uint8_t> keyData = hexToBytes("14aa993c4bb8cfc0893ac0be584bc41e48537c67cd31e0c0659ff18235c7cdd3");
	std::array<uint8_t, Cryptography::CipherKeySize> expectedKey = vectorToArray<Cryptography::CipherKeySize>(keyData);
	...
	Noise::CipherState cipherState;
	vectorToArray(vectorToArray<Cryptography::CipherKeySize>(hexToBytes("0b0a0c00112233445566778899AABBCCDDEEFFFEDCBA9876543210AABBCCDDEE")), cipherState.cipherKey.raw);
	cipherState.nonce = 10;
	Noise::Utils::initializeKey(std::move(newKey), cipherState);
